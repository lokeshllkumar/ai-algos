...
def branch_and_bound_heuristic_visual(graph, start_node, goal_node, heuristic):
    visited = set()
    priority_queue = []
    
    heappush(priority_queue, (0 + heuristic[start_node], 0, [start_node]))
    
    pos = nx.spring_layout(graph)
    plt.figure(figsize=(8, 6))

    while priority_queue:
        f_cost, cumulative_cost, path = heappop(priority_queue)
        current_node = path[-1]

        draw_graph(graph, path, visited, pos, f_cost)
...
def draw_graph(graph, path, visited, pos, f_cost):
    plt.clf()
    ...
    plt.title(f"Branch and Bound with Heuristics - : {f_cost}")
    plt.pause(1)
...
start_node = 'A'
goal_node = 'J'
branch_and_bound_heuristic_visual(graph, start_node, goal_node, heuristic)
