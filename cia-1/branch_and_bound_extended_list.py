...
def branch_and_bound_extended_list(graph, start_node, goal_node):
    extended_list = set()
    priority_queue = []
    ...
    while priority_queue:
        ...
        draw_graph(graph, path, extended_list, pos, cumulative_cost)
...
def draw_graph(graph, path, extended_list, pos, cost):
    plt.clf()
    ...
    plt.title(f"Branch and Bound with Extended List - Cumulative Cost: {cost}")
    plt.pause(1)
...
start_node = 'A'
goal_node = 'J'
branch_and_bound_extended_list(graph, start_node, goal_node)
